...
import io.ktor.http.*
import me.kuku.telegram.entity.StepEntity
...
object LeXinStepLogic {
    ...
    suspend fun login(phone: String, password: String): StepEntity {
        val newPassword = if (password.length == 32) password else password.md5()
        val response = client.post("https://sports.lifesense.com/sessions_service/login?screenHeight=2267&screenWidth=1080&systemType=2&version=4.5") {
            setBody("{\"password\":\"$newPassword\",\"clientId\":\"${UUID.randomUUID()}\",\"appType\":6,\"loginName\":\"$phone\",\"roleType\":0}")
        ...
        return if (jsonNode["code"].asInt() == 200) {
            ...
            StepEntity().also {
                ...
                it.leXinUserid = jsonNode.get("data")["userId"].asText()
                it.leXinAccessToken = jsonNode.get("data")["accessToken"].asText()
            }
        } else error(jsonNode["msg"].asText())
    }

    suspend fun modifyStepCount(stepEntity: StepEntity, step: Int) {
        if (stepEntity.leXinCookie.isEmpty()) error("未绑定乐心运动，操作失败")
        val dateTimePattern = "yyyy-MM-dd hh:mm:ss"
        val datePattern = "yyyy-MM-dd"
        val time = System.currentTimeMillis()
        val second = System.currentTimeMillis() / 1000
        val now = LocalDateTime.now()
        val jsonNode = client.post("https://sports.lifesense.com/sport_service/sport/sport/uploadMobileStepV2?country=%E4%B8%AD%E5%9B%BD&city=%E6%9F%B3%E5%B7%9E&cityCode=450200&timezone=Asia%2FShanghai&latitude=24.368694&os_country=CN&channel=qq&language=zh&openudid=&platform=android&province=%E5%B9%BF%E8%A5%BF%E5%A3%AE%E6%97%8F%E8%87%AA%E6%B2%BB%E5%8C%BA&appType=6&requestId=${UUID.randomUUID()}&countryCode=&systemType=2&longitude=109.532216&devicemodel=V1914A&area=CN&screenwidth=1080&os_langs=zh&provinceCode=450000&promotion_channel=qq&rnd=3d51742c&version=4.6.7&areaCode=450203&requestToken=${UUID.randomUUID()}&network_type=wifi&osversion=10&screenheight=2267&ts=$second") {
            setBody("{\"list\":[{\"active\":1,\"calories\":${step / 4},\"created\":\"${now.format(DateTimeFormatter.ofPattern(dateTimePattern))}\",\"dataSource\":2,\"dayMeasurementTime\":\"${now.format(DateTimeFormatter.ofPattern(datePattern))}\",\"deviceId\":\"M_NULL\",\"distance\":${step / 3},\"id\":\"${UUID.randomUUID()}\",\"isUpload\":0,\"measurementTime\":\"${now.format(DateTimeFormatter.ofPattern(dateTimePattern))}\",\"priority\":0,\"step\":$step,\"type\":2,\"updated\":$time,\"userId\":\"${stepEntity.leXinUserid}\",\"DataSource\":2,\"exerciseTime\":0}]}")
            contentType(ContentType.Application.Json)
            headers { append("Cookie", stepEntity.leXinCookie) }
        }.body<JsonNode>()
...
object XiaomiStepLogic {
    ...
    suspend fun login(phone: String, password: String): StepEntity {
        ...
        val response = client.submitForm("https://api-user.huami.com/registrations/%2B86$phone/tokens",
            formParameters = parameters { map.forEach { (t, u) -> append(t, u) } }) {
            headers { userAgent(ua) }
        }
        val locationUrl = response.headers["location"] ?: error("登录失败")
        val access = RegexUtils.extract(locationUrl, "access=", "&") ?: error("账号或者密码错误")
        ...
        val jsonNode = client.submitForm("https://account.huami.com/v2/client/login",
            formParameters = parameters { ssMap.forEach { (t, u) -> append(t, u) } }) {
            headers { userAgent(ua) }
        }.body<JsonNode>()
        val token = jsonNode["token_info"]["login_token"].asText()
        return StepEntity().also {
            it.miLoginToken = token
        }
    }

    private suspend fun getInfo(token: String): MiInfo {
        val jsonNode = client.get("https://account-cn.huami.com/v1/client/app_tokens?app_name=com.xiaomi.hm.health&dn=api-user.huami.com%2Capi-mifit.huami.com%2Capp-analytics.huami.com&login_token=$token") {
            userAgent(ua)
        }.body<JsonNode>()
        return if ("ok" == jsonNode["result"].asText()) {
            val infoJsonNode = jsonNode.get("token_info")
            MiInfo(infoJsonNode["app_token"].asText(), infoJsonNode["user_id"].asText())
        } else error("登录已失效，请重新登录！！")
    }

    suspend fun modifyStepCount(stepEntity: StepEntity, step: Int) {
        val token = stepEntity.miLoginToken
        if (token.isEmpty()) error("未绑定小米运动，操作失败")
        val miInfo = getInfo(token)
        ...
        val paramsMap = mapOf("userid" to miInfo.userId, "last_sync_data_time" to tenDateStr,
            ...
            """.trimIndent().toUrlDecode()
        )
        val jsonNode = client.submitForm("https://api-mifit-cn.huami.com/v1/data/band_data.json?&t=" + System.currentTimeMillis(),
            parameters { paramsMap.forEach { append(it.key, it.value) } }) {
            headers {
                append("apptoken", miInfo.appToken)
                userAgent(ua)
            }
        }.body<JsonNode>()
        when (jsonNode["code"].asInt()) {
            1 -> return
            0 -> error("步数修改失败，登录已失效")
            else -> error("步数修改失败，未知错误")
        }
    }

}

data class MiInfo(val appToken: String, val userId: String)